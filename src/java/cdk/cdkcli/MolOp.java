...
public class MolOp {
    ...
    private static int calcValence(IAtom atom) {
        ...
        for (IBond bond : atom.bonds()) {
            ...
            if (order != null && order != IBond.Order.UNSET) v +=
                order.numeric();
        }
        return v;
    }
    ...
    private static boolean isDativeAcceptor(IAtom a, DativeBond opt) {
        if (Elements.isMetal(a)) return true;
        if (opt == DativeBond.Metals) return false;
        switch (a.getAtomicNumber()) {
            case IAtom.B:
                return a.getFormalCharge() == 0 && calcValence(a) == 4;
            case IAtom.O:
    ...
    private static boolean isNegDativeAcceptor(IAtom a, DativeBond opt) {
        if (a.getFormalCharge() != -1) return false;
        if (Elements.isMetal(a)) return true;
        if (opt == DativeBond.Metals) return false;
        switch (a.getAtomicNumber()) {
            case IAtom.B:
        ...
        for (IAtom atom : mol.atoms()) {
            ...
            switch (atom.getAtomicNumber()) {
                case 6:
                    if (q == 0) {
                        v = calcValence(atom);
                        if (v == 2) mol.addSingleElectron(mol.indexOf(atom));
                        if (v < 4) mol.addSingleElectron(mol.indexOf(atom));
                    }
                    break;
                case 7:
                    if (q == 0) {
                        v = calcValence(atom);
                        if (v < 3) mol.addSingleElectron(mol.indexOf(atom));
                    }
                    break;
                case 8:
                    if (q == 0) {
                        v = calcValence(atom);
                        if (v < 2) mol.addSingleElectron(mol.indexOf(atom));
                        if (v < 1) mol.addSingleElectron(mol.indexOf(atom));
                    }
                    break;
            }
        }
    ...
    public static void perceiveDativeBonds(IAtomContainer mol, DativeBond opt) {
        if (opt == DativeBond.Never) return;
        for (IBond bond : mol.bonds()) {
            ...
            if (isPosDativeDonor(end, opt) && isNegDativeAcceptor(beg, opt)) {
                ...
                isPosDativeDonor(beg, opt) && isNegDativeAcceptor(end, opt)
            ) {
